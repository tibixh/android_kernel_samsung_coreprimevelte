...
static int sec_nfc_ldo_onoff(struct device *dev, bool onoff)
{
	...
	vdd = devm_regulator_get(dev, "sec-nfc,vdd");
	if (IS_ERR(vdd)) {
		pr_err("[NFC] %s, cannot get vdd\n", __func__);
		return -ENOMEM;
	}

	regulator_set_voltage(vdd, 1800000, 1850000);
...
#ifdef CONFIG_SEC_NFC_LDO_CONTROL
	sec_nfc_ldo_onoff(&client->dev, ON);
#endif
...
static int sec_nfc_set_pinctrl(struct sec_nfc_info *info, int on)
{
	struct pinctrl_state *pinctrl_state;
	int ret = 0;

	if(!info->nfc_pinctrl) {
		pr_err("%s pinctrl not set\n", __func__);
		return 0;
	}

	pinctrl_state = on ? info->nfc_active : info->nfc_suspend;
	if(!pinctrl_state) {
		pr_err("%s:no pinctrl\n", __func__);
	}
	ret = pinctrl_select_state(info->nfc_pinctrl, pinctrl_state);
	if (unlikely(ret)) {
		pr_err("%s:can't set pinctrl %s state\n", __func__,
			on ? "active" : "suspend");
	}
	pr_info("sangam dbg : selected state\n");
	return ret;
}
...
static int sec_nfc_suspend(struct device *dev)
{
	...
	ret = sec_nfc_set_pinctrl(info, 0);
	if (ret)
		pr_err("%s fail to nfc pinctrl\n", __func__);
...
static int sec_nfc_resume(struct device *dev)
{
	...
	pr_info("%s.. \n", __func__);
	ret = sec_nfc_set_pinctrl(info, 1);
	if (ret)
		pr_err("%s fail to nfc pinctrl\n", __func__);
...
#ifdef CONFIG_SEC_NFC_GPIO_LDO_CONTROL
	pdata->ldo_en = of_get_named_gpio_flags(np, "sec-nfc,ldo-gpio",
...
#endif
	pr_info("[NFC] %s  ven-gpio[%d] firm-gpio[%d] wake-gpio[%d] irq[%d]\n",
...
static int sec_nfc_pinctrl(struct device *dev)
{
	...
	if (IS_ERR(nfc_pinctrl)) {
		...
		info->nfc_pinctrl = nfc_pinctrl;
		info->nfc_suspend = pinctrl_lookup_state(info->nfc_pinctrl, "nfc_suspend");
		if (IS_ERR(info->nfc_suspend)) {
			pr_info("%s fail to suspend lookup_state\n", __func__);
			goto err_exit;
		}
		info->nfc_active = pinctrl_lookup_state(info->nfc_pinctrl, "nfc_active");
		if (IS_ERR(info->nfc_active)) {
...
//	disable_irq(g_nfc_info->i2c_info.i2c_dev->irq);
	...
	pr_info("NFC_TEST: fw_ver: %02X %02X %02X, mode: %d\n", buf[0], buf[1],
						buf[2], g_nfc_info->mode);

exit:
	msleep(10);
	sec_nfc_set_mode(g_nfc_info, old_mode);
//	enable_irq(g_nfc_info->i2c_info.i2c_dev->irq);
	msleep(100);
...
#ifdef CONFIG_SEC_NFC_GPIO_LDO_CONTROL
	if (pdata->ldo_en) {
		ret = gpio_request(pdata->ldo_en, "ldo-gpio");
		if (ret) {
			dev_err(dev, "[NFC] failed to get gpio ldo\n");
			goto err_gpio_ldo;
		}
		if(!lpcharge)
			gpio_direction_output(pdata->ldo_en, 1);
		else
